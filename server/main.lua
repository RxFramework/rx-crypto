...
local function RefreshCrypto()
    ...
    if result ~= nil and result[1] ~= nil then
        ...
        if result[1].history ~= nil then
            ...
            TriggerClientEvent('rx-crypto:client:UpdateCryptoWorth', -1, coin, result[1].worth, json.decode(result[1].history))
        else
            TriggerClientEvent('rx-crypto:client:UpdateCryptoWorth', -1, coin, result[1].worth, nil)
        end
    end
end
...
RXCore.Commands.Add('setcryptoworth', 'Set crypto value', { { name = 'crypto', help = 'Name of the crypto currency' }, { name = 'Value', help = 'New value of the crypto currency' } }, false, function(source, args)
    local src = source
    ...
    if crypto ~= nil then
        if Crypto.Worth[crypto] ~= nil then
            ...
            if NewWorth ~= nil then
                ...
                TriggerClientEvent('RXCore:Notify', src, 'You have changed the value of ' .. Crypto.Labels[crypto] .. ' from: $' .. Crypto.Worth[crypto] .. ' to: $' .. NewWorth .. ' (' .. ChangeLabel .. ' ' .. PercentageChange .. '%)')
                Crypto.Worth[crypto] = NewWorth
                TriggerClientEvent('rx-crypto:client:UpdateCryptoWorth', -1, crypto, NewWorth)
                ...
                TriggerClientEvent('RXCore:Notify', src, Lang:t('text.you_have_not_given_a_new_value', { crypto = Crypto.Worth[crypto] }))
            end
        else
            TriggerClientEvent('RXCore:Notify', src, Lang:t('text.this_crypto_does_not_exist'))
        end
    else
        TriggerClientEvent('RXCore:Notify', src, Lang:t('text.you_have_not_provided_crypto_available_qbit'))
    end
end, 'admin')

RXCore.Commands.Add('checkcryptoworth', '', {}, false, function(source)
    local src = source
    TriggerClientEvent('RXCore:Notify', src, Lang:t('text.the_qbit_has_a_value_of', { crypto = Crypto.Worth['qbit'] }))
end)

RXCore.Commands.Add('crypto', '', {}, false, function(source)
    local src = source
    local Player = RXCore.Functions.GetPlayer(src)
    local MyPocket = math.ceil(Player.PlayerData.money.crypto * Crypto.Worth['qbit'])

    TriggerClientEvent('RXCore:Notify', src, Lang:t('text.you_have_with_a_value_of', { playerPlayerDataMoneyCrypto = Player.PlayerData.money.crypto, mypocket = MyPocket }))
end)

-- Events

RegisterServerEvent('rx-crypto:server:FetchWorth', function()
    for name, _ in pairs(Crypto.Worth) do
        ...
        if result[1] ~= nil then
            ...
            if result[1].history ~= nil then
                ...
                TriggerClientEvent('rx-crypto:client:UpdateCryptoWorth', -1, name, result[1].worth, json.decode(result[1].history))
            else
                TriggerClientEvent('rx-crypto:client:UpdateCryptoWorth', -1, name, result[1].worth, nil)
            end
        end
    end
end)

RegisterServerEvent('rx-crypto:server:ExchangeFail', function()
    local src = source
    local Player = RXCore.Functions.GetPlayer(src)
    local ItemData = Player.Functions.GetItemByName('cryptostick')
    if ItemData ~= nil then
        exports['rx-inventory']:RemoveItem(src, 'cryptostick', 1, false, 'rx-crypto:server:ExchangeFail')
        TriggerClientEvent('rx-inventory:client:ItemBox', src, RXCore.Shared.Items['cryptostick'], 'remove')
        TriggerClientEvent('RXCore:Notify', src, Lang:t('error.cryptostick_malfunctioned'), 'error')
    end
end)

RegisterServerEvent('rx-crypto:server:Rebooting', function(state, percentage)
    Crypto.Exchange.RebootInfo.state = state
    Crypto.Exchange.RebootInfo.percentage = percentage
end)

RegisterServerEvent('rx-crypto:server:GetRebootState', function()
    local src = source
    TriggerClientEvent('rx-crypto:client:GetRebootState', src, Crypto.Exchange.RebootInfo)
end)

RegisterServerEvent('rx-crypto:server:SyncReboot', function()
    TriggerClientEvent('rx-crypto:client:SyncReboot', -1)
end)

RegisterServerEvent('rx-crypto:server:ExchangeSuccess', function(LuckChance)
    local src = source
    local Player = RXCore.Functions.GetPlayer(src)
    local ItemData = Player.Functions.GetItemByName('cryptostick')
    if ItemData ~= nil then
        ...
        exports['rx-inventory']:RemoveItem(src, 'cryptostick', 1, false, 'rx-crypto:server:ExchangeSuccess')
        Player.Functions.AddMoney('crypto', Amount, 'rx-crypto:server:ExchangeSuccess')
        TriggerClientEvent('RXCore:Notify', src, Lang:t('success.you_have_exchanged_your_cryptostick_for', { amount = Amount }), 'success', 3500)
        TriggerClientEvent('rx-inventory:client:ItemBox', src, RXCore.Shared.Items['cryptostick'], 'remove')
        TriggerClientEvent('rx-phone:client:AddTransaction', src, Player, {}, Lang:t('credit.there_are_amount_credited', { amount = Amount }), 'Credit')
    end
end)

-- Callbacks

RXCore.Functions.CreateCallback('rx-crypto:server:HasSticky', function(source, cb)
...
RXCore.Functions.CreateCallback('rx-crypto:server:GetCryptoData', function(source, cb, name)
    local Player = RXCore.Functions.GetPlayer(source)
    local CryptoData = {
...
RXCore.Functions.CreateCallback('rx-crypto:server:BuyCrypto', function(source, cb, data)
    local Player = RXCore.Functions.GetPlayer(source)
    ...
    if Player.PlayerData.money.bank >= total_price then
        ...
        TriggerClientEvent('rx-phone:client:AddTransaction', source, Player, data, Lang:t('credit.you_have_qbit_purchased', { dataCoins = tonumber(data.Coins) }), 'Credit')
...
RXCore.Functions.CreateCallback('rx-crypto:server:SellCrypto', function(source, cb, data)
    local Player = RXCore.Functions.GetPlayer(source)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        TriggerClientEvent('rx-phone:client:AddTransaction', source, Player, data, Lang:t('debit.you_have_sold', { dataCoins = tonumber(data.Coins) }), 'Debit')
...
RXCore.Functions.CreateCallback('rx-crypto:server:TransferCrypto', function(source, cb, data)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        if result[1] ~= nil then
            ...
            TriggerClientEvent('rx-phone:client:AddTransaction', source, Player, data, 'You have ' .. tonumber(data.Coins) .. " RX Coin(s) transferred!", 'Debit')
            local Target = RXCore.Functions.GetPlayerByCitizenId(result[1].citizenid)

            if Target ~= nil then
                Target.Functions.AddMoney('crypto', tonumber(data.Coins), 'transfer crypto')
                TriggerClientEvent('rx-phone:client:AddTransaction', Target.PlayerData.source, Player, data, 'There are ' .. tonumber(data.Coins) .. " RX Coin(s) credited!", 'Credit')
            else
